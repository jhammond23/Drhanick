...
const Gallery = () => {
  ...
  // Slug helper for procedure-level anchors (ex: "Rhinoplasty" -> "rhinoplasty")
  const slugify = (str = "") =>
    str
      .toLowerCase()
      .replace(/[®™]/g, "")
    ...
    () => [
      {
        ...
        procedures: [
          ...
          {
            title: "Facelift Side-by-Side Views",
            items: [
        ...
        procedures: [
          {
            ...
            items: [
              { placeholder: true, label: "Add image" },
              { placeholder: true, label: "Add image" }
            ]
          },
          {
  ...
  // NEW: if URL has a hash like #nose-rhinoplasty, scroll to that procedure card
  useEffect(() => {
    const scrollToHash = () => {
    ...
    <div className="gallery-page">
      ...
      <header className="gallery-hero">
        <div className="gallery-hero__inner">
          ...
          <p>
            This gallery contains photos illustrating
surgical procedures and may not be suitable for all visitors, all photos are used with
written patient permission, images are not retouched, and are only intended to
provide examples of obtained results.
          </p>
        </div>
      </header>
      ...
      <div className="gallery-disclaimer">
        <p>Explore real patient results organized by treatment area. Click any image to view it
            larger. Use the quick links above to jump between sections.</p>
      </div>
        ...
        {sections.map((section, sIdx) => (
          ...
          >
            ...
            {section.procedures.map((proc, pIdx) => {
              const procId = `${section.id}-${slugify(proc.title)}`;
              return (
                <article
                    ...
                    {proc.items.map((item, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {badge ? (
                            <span className={`img-badge ${badge.toLowerCase()}`}>{badge}</span>
                          ) : null}
                        </button>
                      );
                    })}
